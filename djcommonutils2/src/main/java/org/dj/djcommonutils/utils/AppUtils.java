...
public class AppUtils {
    ...
    public static String getDeviceSN(Context context) {
            ...
            synchronized (AppUtils.class) {
                if (null == IMSI) {
                    try {
                        ...
                        if (tm != null) IMSI = tm.getDeviceId();
                        if (TextUtils.isEmpty(IMSI)) {
                            //2.sim serial number
                            if (tm != null) IMSI = tm.getSimSerialNumber();//对于cdma设备，返回的是一个空值！
                            if (TextUtils.isEmpty(IMSI)) {
                                //3.IMSI = SubscriberId
                                if (tm != null) IMSI = tm.getSubscriberId();
                                if (TextUtils.isEmpty(IMSI)) {
                                    //4.android_id
        ...
        if (TextUtils.isEmpty(IMSI)) {
            ...
            IMSI = sp.getString("tempIMSI", null);
            if (TextUtils.isEmpty(IMSI)) {
                //得到时间
    ...
    public static int compareVersion(String versionServer, String versionLocal) {
        ...
        if(TextUtils.isEmpty(versionServer) || TextUtils.isEmpty(versionServer.trim())) {
            return 0;
        }

        if (TextUtils.isEmpty(versionLocal) || TextUtils.isEmpty(versionLocal.trim())) {
            return 0;
        }
