package org.dj.djcommonutils.view.wheelview;
...
public class DateTimePicker extends WheelPicker {
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        if (timeMode != NONE) {
            selectedHour = WheelDateUtils.fillZero(hour);
            selectedMinute = WheelDateUtils.fillZero(minute);
        }
    }
    ...
    protected View makeCenterView() {
        ...
        if (dateMode != NONE && months.size() == 0) {
            int selectedYear = WheelDateUtils.trimZero(getSelectedYear());
            changeMonthData(selectedYear);
        }
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
                selectedYear = WheelDateUtils.trimZero(getSelectedYear());
            } else {
                selectedYear = Calendar.getInstance(Locale.CHINA).get(Calendar.YEAR);
            }
            int selectedMonth = WheelDateUtils.trimZero(getSelectedMonth());
        ...
        if (timeMode != NONE && minutes.size() == 0) {
            changeMinuteData(WheelDateUtils.trimZero(selectedHour));
        }
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    //需要根据年份及月份动态计算天数
                    int selectedYear = WheelDateUtils.trimZero(selectedYearStr);
                    changeMonthData(selectedYear);
                    monthView.setItems(months, selectedMonthIndex);
                    if (onWheelListener != null) {
                        onWheelListener.onMonthWheeled(selectedMonthIndex, months.get(selectedMonthIndex));
                    }
                    changeDayData(selectedYear, WheelDateUtils.trimZero(months.get(selectedMonthIndex)));
            ...
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                        ...
                        changeDayData(selectedYear, WheelDateUtils.trimZero(selectedMonthStr));
                        dayView.setItems(days, selectedDayIndex);
                        if (onWheelListener != null) {
                            onWheelListener.onDayWheeled(selectedDayIndex, days.get(selectedDayIndex));
                        }
    ...
    private void initYearData() {
        ...
        if (!resetWhileWheel) {
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                int index = years.indexOf(WheelDateUtils.fillZero(Calendar.getInstance().get(Calendar.YEAR)));
                if (index == -1) {
    ...
    private void changeMonthData(int selectedYear) {
        ...
        if (!resetWhileWheel) {
            if (months.size() > selectedMonthIndex) {
                ...
                preSelectMonth = WheelDateUtils.fillZero(Calendar.getInstance().get(Calendar.MONTH) + 1);
            }
        }
        months.clear();
        ...
        if (startYear == endYear) {
            if (startMonth > endMonth) {
                for (int i = endMonth; i >= startMonth; i--) {
                    months.add(WheelDateUtils.fillZero(i));
                }
            } else {
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        int maxDays = WheelDateUtils.calculateDaysInMonth(selectedYear, selectedMonth);
        String preSelectDay = "";
        if (!resetWhileWheel) {
            ...
            if (days.size() > selectedDayIndex) {
                ...
                preSelectDay = WheelDateUtils.fillZero(Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
            }
        }
        days.clear();
        if (selectedYear == startYear && selectedMonth == startMonth
            ...
            for (int i = startDay; i <= endDay; i++) {
                days.add(WheelDateUtils.fillZero(i));
            }
    ...
    private void initHourData() {
        ...
        for (int i = startHour; i <= endHour; i++) {
            String hour = WheelDateUtils.fillZero(i);
            if (!resetWhileWheel) {
                if (i == currentHour) {
                    selectedHour = hour;
                }
        ...
        if (!resetWhileWheel) {
            selectedMinute = WheelDateUtils.fillZero(Calendar.getInstance().get(Calendar.MINUTE));
        }
    }
    ...
    private void changeMinuteData(int selectedHour) {
        ...
        if (startHour == endHour) {
            ...
            for (int i = startMinute; i <= endMinute; i++) {
                minutes.add(WheelDateUtils.fillZero(i));
            }
